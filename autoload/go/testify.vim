...
function! go#testify#Test(bang, compile, ...) abort
  ...
  if l:err != 0
    ...
    call go#util#EchoError("[testify] FAIL")
  else
    call go#list#Clean(l:listtype)

    if a:compile
      call go#util#EchoSuccess("[testify] SUCCESS")
    else
      call go#util#EchoSuccess("[testify] PASS")
    endif
  endif
...
function! go#testify#Func(bang, ...) abort
  ...
  call call('go#testify#Test', args)
endfunction
